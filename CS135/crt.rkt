...
(define (CRT-exists? cong-sys)
  (if (or(equal? cong-sys '())(equal? (cdr cong-sys) '()))
    #t
  (if(= (euclid-gcd(car(cdr(car cong-sys)))(car(cdr(car(cdr cong-sys)))))1)
        (and(CRT-exists?(cons(car cong-sys)(cdr(cdr cong-sys))))(CRT-exists?(cdr cong-sys)))
         #f)))
...
(define (m cong-sys)
  (apply * (secondnum cong-sys)))


  (define (secondnum cong-sys)
  (map cadr cong-sys))
...
(define (help x y)
  (if (null? x)
      '()
       (append(list(*(*(/ y(car(cdr(car x))))(car (car x)))(mul-inv(/ y(car(cdr(car x))))(car(cdr (car x))))))(help(cdr x)y))))
