...
(define (encrypt plaintext key)
  (helper(string->list plaintext) key))


(define(helper listL key)
  (if(null? listL)
     '()
     (append(list(mod-exp(char->integer(car listL))(cadr key)(car key)))(helper(cdr listL)key))))
...
(define (decrypt ciphertext key)
  (if(equal? ciphertext '())
     ""
     (string-append(list->string(list(integer->char(mod-exp(car ciphertext)(car(cdr key))(car key)))))(decrypt(cdr ciphertext)key))))
