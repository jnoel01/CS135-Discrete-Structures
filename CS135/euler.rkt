...
(define (degree G v)
  (if(equal? G '())
     0
  (if(and(equal? v (caar G))(equal? v (cadar G)))
         (+ 2 (degree (cdr G) v))
  (if(or(equal? v (caar G))(equal? v (cadar G)))
     (+ 1 (degree (cdr G) v))
     (degree (cdr G) v)))))
...
(define (all-even? G)
  (helperE(helper G(get-vertices G))))
 
(define(helperE G)
  (if(null? G)
     #t
   (if(not(eq? 0 (modulo (car G) 2)))
    #f
    (helperE(cdr G)))))
...
(define (helper G v)
  (if(null? v)
         '()
          (append (list (degree G (car v)))(helper G (cdr v)))))
...
(define (adjacent G v)
  (if(equal? G'())
   ...
   (if (equal? (car(car G)) v)
     (cons (car(cdr(car G))) (adjacent (cdr G) v))
     (if(equal? (car(cdr(car G))) v)
     (cons (car(car G)) (adjacent (cdr G) v))
     (adjacent(cdr G) v)))))
