...
(define (find-top G)
  (if(equal? (get-vertices G) '())
     (tsort-error)
  (if(equal? (indegree(get-edges G)(car(get-vertices G))) 0)
     (car(get-vertices G))
     (find-top (make-graph (cdr(get-vertices G)) (get-edges G))))))
...
(define (tsort G)
  (if (null? (get-vertices G))
        '()
        (append(list(find-top G))(tsort(pop-vertex G(find-top G))))))
