...
(define (power R k)
  ...
  (if (equal? k 1)
      R
  (make-set(compose(power R(- k 1)) R)))))
...
(define (transitive-closure R)
 ...
 (define(helperfuncA R n)
   (if (equal? n (+ (cardinality R) 1))
              '()
              (union(power R n) (helperfuncA R (+ n 1)))))
